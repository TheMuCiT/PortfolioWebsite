...
const Animation = () => {
  ...
  const scrollAnimation = () => {
    ...
    images[0].style.transform = `translateY(${-WindowsPosition * 3}%)`
    images[0].style.filter = `drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)) `

    images[1].style.transform = `translateY(${-WindowsPosition * 2.5}%) scale(0.95)`
    ...
    images[3].style.transform = `translateY(${-WindowsPosition * 1.5}%) scale(0.85)`
    images[3].style.filter = `drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)) blur(0.2px)`

    if (images[4]) {
      images[4].style.transform = `translateY(${-WindowsPosition * 1}%) scale(0.8)`
      images[4].style.filter = `drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)) blur(0.25px)`
    }

    if (images[5]) {
      images[5].style.transform = `translateY(${-WindowsPosition / 1.5}%) scale(0.75)`
      images[5].style.filter = `drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)) blur(0.3px)`
    }

    if (images[6]) {
      images[6].style.transform = `translateY(${-WindowsPosition / 2}%) scale(0.7)`
      images[6].style.filter = `drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)) blur(0.35px)`
    }

    if (images[7]) {
      images[7].style.transform = `translateY(${-WindowsPosition / 2.5}%) scale(0.65)`
